...
target and non-target maps along each gradient (1-5) dimension of state space.

Scores calculate using spearman rank (state space package)
"
####################### Load libraries #########################################
...
setwd("C:\\Users\\bront\\Documents\\CanadaPostdoc\\GradientSpace\\results\\scientificreports\\1st_revision\\lmm_target_to_nontarget_maps")

results_dir <- "C:\\Users\\bront\\Documents\\CanadaPostdoc\\GradientSpace\\results\\scientificreports\\1st_revision\\lmm_target_to_nontarget_maps"
...
cope3.contrasts
emmean3
cope4.contrasts <- pairs(emmean4, adjust = "bonferroni", infer = TRUE)
cope4.contrasts
emmean4
...
cat("Comparing maps along gradient 4:\n", file = fp2, append = TRUE)
capture.output(cope4.contrasts,file = fp2, append = TRUE)
cat("\n\n\n", file = fp2, append = TRUE)
...
bar4 <- bar4 + theme(axis.text.x=element_text(size = fontsize, color = "black"))+
...
# put together
targetnontarget_bar_plots <- (bar1)/(bar2)/(bar3)/(bar4)&
