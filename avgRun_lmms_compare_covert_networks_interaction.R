...
to compare networks between covert experiential states.
"
####################### Load libraries #########################################
...
######################### Data prep ############################################
# select covert states
df1 <- subset(df1, (task_id %in% c("pca1", "pca2","pca3")))

## Set fixed factors: task_id 
df1$task_id <- factor(df1$task_id, levels = c("pca1", "pca2","pca3"),
                         ordered = FALSE) #set order to False.
#to check it's worked, run this command and see print out.
levels(df1$task_id)

# set network as fixed factor
df1$yeo_network <- factor(df1$yeo_network,levels = c("Default Mode",
                                                     "Somatomotor",
                                                     "Limbic",
                                                     "Visuospatial",
                                                     "Dorsal Attention",
...
fp = "LMM_avgRun_covert_networks.txt"
...
############################# Models ###########################################
# set up list of dependent variables (just 1 this time)
dv <- c("mean_value")

# set col index for creating bootstrap tables in loop below
col_index <- c(2,3,4)

# first run all 3 models without lmertest and use bootstrapping to calculate
# parameter estimates
...
myfile2 <- file.path(results_dir,"LMM_avgRun_thoughts_network_summary")
...
myfile3 <- file.path(results_dir,"LMM_avgRun_thoughts_network_bootstrap")
...
myfile3 <- file.path(results_dir,"LMM_avgRun_thoughts_network_anova")
